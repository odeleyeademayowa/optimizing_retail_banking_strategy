...
# Check if file is uploaded or use default sample data
if uploaded_file:
    df = pd.read_csv(uploaded_file)
    st.dataframe(df.head())
    st.success("✅ File uploaded successfully!")
else:
    # Use the sample dataset (make sure `cluster_rfm.csv` is in the same directory as your Streamlit app)
    df = pd.read_csv("cluster_rfm.csv")
    st.info("ℹ️ No file uploaded. Using default sample dataset (cluster_rfm.csv)")
